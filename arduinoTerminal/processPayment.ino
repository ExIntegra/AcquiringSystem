                    ...
                    String uid_String)   ///< uidString - уникальный номер rdif-метки (банковской карты)
{
  ...
  if(statusTransaction && blockIsCart == false && pincodeNeed == false)       // Если транзакция одобрена, карта не заблокирована и пинкода не нужен, то оплата произведена.
  {
    displayMessage("Payment", true, 4, 0);                    // Выводим сообщение.
    displayMessage("successful!", false, 2, 1);               // Выводим сообщение.
  }

  else if(statusTransaction && blockIsCart == false && pincodeNeed)  // Если транзакция одобрена, карта не заблокирована и пинкод нужен, то вводим пинкод.
  {
