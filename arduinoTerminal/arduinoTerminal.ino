...
PaymentProcess terminalArduinoUNO;          // Создаем экземпляр класса

void setup(){                               // Разовый запуск функции для установки значений
...
void loop(){
  terminalArduinoUNO.automatonOfStates();   // Вызываем метод, который реализует автомат состояний
}